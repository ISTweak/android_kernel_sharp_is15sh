...
#define SH_ACPU_MIN_UV_MV 800U
#define SH_ACPU_MAX_UV_MV 1500U
...
static struct clkctl_acpu_speed acpu_freq_tbl[] = {
	...
	{ 1,  122880, PLL_3, 5, 5,  61440000, 1000, VDD_RAW(950) },
	...
	{ 1,  368640, PLL_3, 5, 1, 122800000, 1050, VDD_RAW(1000) },
...
void acpuclk_set_vdd(unsigned int khz, int vdd)
{
	...
	for (i = 0; acpu_freq_tbl[i].acpu_clk_khz; i++) {
		if(acpu_freq_tbl[i].use_for_scaling==1) {
			if (khz == 0)
				new_vdd = min(max((acpu_freq_tbl[i].vdd_mv + vdd), SH_ACPU_MIN_UV_MV), SH_ACPU_MAX_UV_MV);
			else if (acpu_freq_tbl[i].acpu_clk_khz == khz)
				new_vdd = min(max((unsigned int)vdd, SH_ACPU_MIN_UV_MV), SH_ACPU_MAX_UV_MV);
			else continue;
